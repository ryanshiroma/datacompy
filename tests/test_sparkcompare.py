...
from pyspark.sql.types import StructType, StructField, DoubleType, StringType, LongType, DateType, DecimalType
...
def base_df1_fixture(spark):
    mock_data = [
        Row(acct=10000001234, dollar_amt=123, name='George Maharis', float_fld=14530.1555,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001235, dollar_amt=0, name='Michael Bluth', float_fld=1.0,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001236, dollar_amt=1345, name='George Bluth', float_fld=None,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001237, dollar_amt=123456, name='Bob Loblaw', float_fld=345.12,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001239, dollar_amt=1, name='Lucille Bluth', float_fld=None,
            date_fld=datetime.date(2017, 1, 1))
    ]
...
def base_df2_fixture(spark):
    mock_data = [
        Row(acct=10000001234, dollar_amt=123, super_duper_big_long_name='George Maharis', float_fld=14530.1555,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001235, dollar_amt=0, super_duper_big_long_name='Michael Bluth', float_fld=1.0,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001236, dollar_amt=1345, super_duper_big_long_name='George Bluth', float_fld=None,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001237, dollar_amt=123456, super_duper_big_long_name='Bob Loblaw', float_fld=345.12,
            date_fld=datetime.date(2017, 1, 1)),
        Row(acct=10000001239, dollar_amt=1, super_duper_big_long_name='Lucille Bluth', float_fld=None,
            date_fld=datetime.date(2017, 1, 1))
    ]
...
def compare_df1_fixture(spark):
    mock_data2 = [
        Row(acct=10000001234, dollar_amt=123.4, name='George Michael Bluth', float_fld=14530.155, accnt_purge=False),
        Row(acct=10000001235, dollar_amt=0.45, name='Michael Bluth', float_fld=None, accnt_purge=False),
        Row(acct=10000001236, dollar_amt=1345.0, name='George Bluth', float_fld=1.0, accnt_purge=False),
        Row(acct=10000001237, dollar_amt=123456.0, name='Bob Loblaw', float_fld=345.12, accnt_purge=False),
...
def compare_df3_fixture(spark):
    mock_data2 = [
        Row(account_identifier=10000001234, dollar_amount=123.4, name='George Michael Bluth', float_field=14530.155,
            date_field=datetime.date(2017, 1, 1), accnt_purge=False),
        Row(account_identifier=10000001235, dollar_amount=0.45, name='Michael Bluth', float_field=1.0,
            date_field=datetime.date(2017, 1, 1), accnt_purge=False),
        Row(account_identifier=10000001236, dollar_amount=1345.0, name='George Bluth', float_field=None,
            date_field=datetime.date(2017, 1, 1), accnt_purge=False),
        Row(account_identifier=10000001237, dollar_amount=123456.0, name='Bob Loblaw', float_field=345.12,
            date_field=datetime.date(2017, 1, 1), accnt_purge=False),
        Row(account_identifier=10000001239, dollar_amount=1.05, name='Lucille Bluth', float_field=None,
...
def base_td_fixture(spark):
    mock_data = [
        Row(acct=10000001234, acct_seq=0, stat_cd='*2', open_dt=datetime.date(2017, 5, 1), cd='0001'),
        Row(acct=10000001235, acct_seq=0, stat_cd='V1', open_dt=datetime.date(2017, 5, 2), cd='0002'),
        Row(acct=10000001236, acct_seq=0, stat_cd='V2', open_dt=datetime.date(2017, 5, 3), cd='0003'),
        Row(acct=10000001237, acct_seq=0, stat_cd='*2', open_dt=datetime.date(2017, 5, 4), cd='0004'),
        Row(acct=10000001238, acct_seq=0, stat_cd='*2', open_dt=datetime.date(2017, 5, 5), cd='0005')
    ]
...
def compare_source_fixture(spark):
    mock_data = [
        Row(ACCOUNT_IDENTIFIER=10000001234, SEQ_NUMBER=0, STATC=None,
            ACCOUNT_OPEN=2017121, CODE=1.0),
        Row(ACCOUNT_IDENTIFIER=10000001235, SEQ_NUMBER=0, STATC='V1',
            ACCOUNT_OPEN=2017122, CODE=2.0),
        Row(ACCOUNT_IDENTIFIER=10000001236, SEQ_NUMBER=0, STATC='V2',
            ACCOUNT_OPEN=2017123, CODE=3.0),
        Row(ACCOUNT_IDENTIFIER=10000001237, SEQ_NUMBER=0, STATC='V3',
            ACCOUNT_OPEN=2017124, CODE=4.0),
        Row(ACCOUNT_IDENTIFIER=10000001238, SEQ_NUMBER=0, STATC=None,
            ACCOUNT_OPEN=2017125, CODE=5.0)
    ]
...
def base_decimal_fixture(spark):
    ...
    return spark.createDataFrame(mock_data, schema=StructType([
...
def comparison1_fixture(base_df1, compare_df1, spark):
    return SparkCompare(spark, base_df1, compare_df1, join_columns=['acct'], cache_intermediates=CACHE_INTERMEDIATES)
...
def comparison3_fixture(base_df1, compare_df3, spark):
    return SparkCompare(spark, base_df1, compare_df3,
                        join_columns=[('acct', 'account_identifier')],
                        column_mapping=[('dollar_amt', 'dollar_amount'),
...
def comparison4_fixture(base_df2, compare_df1, spark):
    return SparkCompare(spark, base_df2, compare_df1, join_columns=['acct'],
...
def test_column_comparison_outputs_number_of_columns_with_differences_for_differently_named_columns(comparison3):
    stdout = six.StringIO()
...
def test_column_comparison_outputs_number_of_columns_with_differences_for_known_diffs(comparison_kd1):
    stdout = six.StringIO()

    comparison_kd1.report(file=stdout)

    assert '****** Column Comparison ******' in stdout.getvalue()
    assert 'Number of columns compared with unexpected differences in some values: 1' in stdout.getvalue()
    assert 'Number of columns compared with all values equal but known differences found: 2' in stdout.getvalue()
    assert 'Number of columns compared with all values completely equal: 0' in stdout.getvalue()


def test_column_comparison_outputs_number_of_columns_with_differences_for_custom_known_diffs(comparison_kd2):
    stdout = six.StringIO()

    comparison_kd2.report(file=stdout)

    assert '****** Column Comparison ******' in stdout.getvalue()
    assert 'Number of columns compared with unexpected differences in some values: 2' in stdout.getvalue()
    assert 'Number of columns compared with all values equal but known differences found: 1' in stdout.getvalue()
    assert 'Number of columns compared with all values completely equal: 0' in stdout.getvalue()
...
def test_columns_with_unequal_values_show_mismatch_counts(comparison1):
    ...
    assert re.search(r"""float_fld \s+ float_fld \s+ double \s+ double \s+ 1 \s+ 3""", stdout.getvalue(), re.X)
    assert re.search(r"""name \s+ name \s+ string \s+ string \s+ 3 \s+ 1""", stdout.getvalue(), re.X)
...
def test_columns_with_different_names_with_unequal_values_show_mismatch_counts(comparison3):
    ...
    assert re.search(r"""float_fld \s+ float_field \s+ double \s+ double \s+ 4 \s+ 1""", stdout.getvalue(), re.X)
...
def test_rows_only_base_returns_a_dataframe_with_rows_only_in_base(spark, comparison1):
    ...
    schema = StructType([StructField("acct", LongType(), True), StructField("date_fld", DateType(), True),
                         ...
                         StructField("float_fld", DoubleType(), True), StructField("name", StringType(), True)])
...
def test_rows_both_mismatch_returns_a_dataframe_with_rows_where_variables_mismatched(spark, comparison1):
    expected_df = spark.createDataFrame([
        Row(acct=10000001234, dollar_amt_base=123, dollar_amt_compare=123.4, dollar_amt_match=False,
            name_base='George Maharis', name_compare='George Michael Bluth', name_match=False,
            float_fld_base=14530.1555, float_fld_compare=14530.155, float_fld_match=False,
            date_fld=datetime.date(2017, 1, 1), accnt_purge=False),
        Row(acct=10000001235, dollar_amt_base=0, dollar_amt_compare=0.45, dollar_amt_match=False,
            name_base='Michael Bluth', name_compare='Michael Bluth', name_match=True,
            float_fld_base=1.0, float_fld_compare=None, float_fld_match=False,
            date_fld=datetime.date(2017, 1, 1), accnt_purge=False),
        Row(acct=10000001236, dollar_amt_base=1345, dollar_amt_compare=1345.0, dollar_amt_match=True,
            name_base='George Bluth', name_compare='George Bluth', name_match=True,
            float_fld_base=None, float_fld_compare=1.0, float_fld_match=False,
            date_fld=datetime.date(2017, 1, 1), accnt_purge=False)
    ])
...
def test_rows_both_mismatch_only_includes_rows_with_true_mismatches_when_known_diffs_are_present(spark, comparison_kd1):
    expected_df = spark.createDataFrame([
        Row(acct=10000001237, acct_seq=0, stat_cd_base='*2', stat_cd_compare='V3',
            stat_cd_match=False, stat_cd_match_type="MISMATCH", open_dt_base=datetime.date(2017, 5, 4),
            open_dt_compare=2017124, open_dt_match=True, open_dt_match_type="KNOWN_DIFFERENCE", cd_base='0004',
            cd_compare=4.0, cd_match=True, cd_match_type="KNOWN_DIFFERENCE")
    ])
...
def test_rows_both_all_returns_a_dataframe_with_all_rows_in_both_dataframes(spark, comparison1):
    expected_df = spark.createDataFrame([
        ...
        Row(acct=10000001236, dollar_amt_base=1345, dollar_amt_compare=1345.0, dollar_amt_match=True,
            ...
            float_fld_base=None, float_fld_compare=1.0, float_fld_match=False,
            date_fld=datetime.date(2017, 1, 1), accnt_purge=False),
        Row(acct=10000001237, dollar_amt_base=123456, dollar_amt_compare=123456.0, dollar_amt_match=True,
            name_base='Bob Loblaw', name_compare='Bob Loblaw', name_match=True,
            float_fld_base=345.12, float_fld_compare=345.12, float_fld_match=True,
            date_fld=datetime.date(2017, 1, 1), accnt_purge=False)
    ])
...
def test_rows_both_all_shows_known_diffs_flag_and_known_diffs_count_as_matches(spark, comparison_kd1):
    expected_df = spark.createDataFrame([
        Row(acct=10000001234, acct_seq=0, stat_cd_base='*2', stat_cd_compare=None,
            stat_cd_match=True, stat_cd_match_type="KNOWN_DIFFERENCE", open_dt_base=datetime.date(2017, 5, 1),
            open_dt_compare=2017121, open_dt_match=True, open_dt_match_type="KNOWN_DIFFERENCE", cd_base='0001',
            cd_compare=1.0, cd_match=True, cd_match_type="KNOWN_DIFFERENCE"),
        Row(acct=10000001235, acct_seq=0, stat_cd_base='V1', stat_cd_compare='V1',
            stat_cd_match=True, stat_cd_match_type="MATCH", open_dt_base=datetime.date(2017, 5, 2),
            open_dt_compare=2017122, open_dt_match=True, open_dt_match_type="KNOWN_DIFFERENCE", cd_base='0002',
            cd_compare=2.0, cd_match=True, cd_match_type="KNOWN_DIFFERENCE"),
        Row(acct=10000001236, acct_seq=0, stat_cd_base='V2', stat_cd_compare='V2',
            stat_cd_match=True, stat_cd_match_type="MATCH", open_dt_base=datetime.date(2017, 5, 3),
            open_dt_compare=2017123, open_dt_match=True, open_dt_match_type="KNOWN_DIFFERENCE", cd_base='0003',
            cd_compare=3.0, cd_match=True, cd_match_type="KNOWN_DIFFERENCE"),
        Row(acct=10000001237, acct_seq=0, stat_cd_base='*2', stat_cd_compare='V3',
            stat_cd_match=False, stat_cd_match_type="MISMATCH", open_dt_base=datetime.date(2017, 5, 4),
            open_dt_compare=2017124, open_dt_match=True, open_dt_match_type="KNOWN_DIFFERENCE", cd_base='0004',
            cd_compare=4.0, cd_match=True, cd_match_type="KNOWN_DIFFERENCE"),
        Row(acct=10000001238, acct_seq=0, stat_cd_base='*2', stat_cd_compare=None,
            stat_cd_match=True, stat_cd_match_type="KNOWN_DIFFERENCE", open_dt_base=datetime.date(2017, 5, 5),
            open_dt_compare=2017125, open_dt_match=True, open_dt_match_type="KNOWN_DIFFERENCE", cd_base='0005',
            cd_compare=5.0, cd_match=True, cd_match_type="KNOWN_DIFFERENCE")
    ])
...
def test_rows_both_all_returns_a_dataframe_with_all_rows_in_identical_dataframes(spark, comparison2):
    expected_df = spark.createDataFrame([
        Row(acct=10000001234, dollar_amt_base=123, dollar_amt_compare=123, dollar_amt_match=True,
            name_base='George Maharis', name_compare='George Maharis', name_match=True,
            float_fld_base=14530.1555, float_fld_compare=14530.1555, float_fld_match=True,
            date_fld_base=datetime.date(2017, 1, 1), date_fld_compare=datetime.date(2017, 1, 1), date_fld_match=True),
        Row(acct=10000001235, dollar_amt_base=0, dollar_amt_compare=0, dollar_amt_match=True,
            name_base='Michael Bluth', name_compare='Michael Bluth', name_match=True,
            float_fld_base=1.0, float_fld_compare=1.0, float_fld_match=True,
            date_fld_base=datetime.date(2017, 1, 1), date_fld_compare=datetime.date(2017, 1, 1), date_fld_match=True),
        Row(acct=10000001236, dollar_amt_base=1345, dollar_amt_compare=1345, dollar_amt_match=True,
            name_base='George Bluth', name_compare='George Bluth', name_match=True,
            float_fld_base=None, float_fld_compare=None, float_fld_match=True,
            date_fld_base=datetime.date(2017, 1, 1), date_fld_compare=datetime.date(2017, 1, 1), date_fld_match=True),
        Row(acct=10000001237, dollar_amt_base=123456, dollar_amt_compare=123456, dollar_amt_match=True,
            name_base='Bob Loblaw', name_compare='Bob Loblaw', name_match=True,
            float_fld_base=345.12, float_fld_compare=345.12, float_fld_match=True,
            date_fld_base=datetime.date(2017, 1, 1), date_fld_compare=datetime.date(2017, 1, 1), date_fld_match=True),
        Row(acct=10000001239, dollar_amt_base=1, dollar_amt_compare=1, dollar_amt_match=True,
            name_base='Lucille Bluth', name_compare='Lucille Bluth', name_match=True,
            float_fld_base=None, float_fld_compare=None, float_fld_match=True,
            date_fld_base=datetime.date(2017, 1, 1), date_fld_compare=datetime.date(2017, 1, 1), date_fld_match=True)
    ])
...
def test_rows_both_all_returns_all_rows_in_both_dataframes_for_differently_named_columns(spark, comparison3):
    expected_df = spark.createDataFrame([
        Row(acct=10000001234, dollar_amt_base=123, dollar_amt_compare=123.4, dollar_amt_match=False,
            name_base='George Maharis', name_compare='George Michael Bluth', name_match=False,
            float_fld_base=14530.1555, float_fld_compare=14530.155, float_fld_match=False,
        ...
        Row(acct=10000001237, dollar_amt_base=123456, dollar_amt_compare=123456.0, dollar_amt_match=True,
            ...
            date_fld_base=datetime.date(2017, 1, 1), date_fld_compare=datetime.date(2017, 1, 1), date_fld_match=True,
            accnt_purge=False),
        Row(acct=10000001239, dollar_amt_base=1, dollar_amt_compare=1.05, dollar_amt_match=False,
            name_base='Lucille Bluth', name_compare='Lucille Bluth', name_match=True,
            float_fld_base=None, float_fld_compare=None, float_fld_match=True,
            date_fld_base=datetime.date(2017, 1, 1), date_fld_compare=datetime.date(2017, 1, 1), date_fld_match=True,
            accnt_purge=True)
...
def text_alignment_validator(report, section_start, section_end, left_indices, right_indices, column_regexes):
