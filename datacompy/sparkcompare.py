...
class SparkCompare(object):
    ...
    def __init__(self, spark_session, df1, df2, join_columns, column_mapping=None,
    ...
    def _get_unq_df1_rows(self):
        ...
        return self.df1.select(self._join_column_names).subtract(self.df2.select(self._join_column_names))

    def _get_unq_df2_rows(self):
        """Get the rows only from df2 data frame"""
        return self.df2.select(self._join_column_names).subtract(self.df1.select(self._join_column_names))
    ...
    def _print_columns_summary(self, myfile):
        ...
        print('Number of columns in df1 but not df2: {}'.format(len(self.columns_only_df1)), file=myfile)
        print('Number of columns in df2 but not df1: {}'.format(len(self.columns_only_df2)), file=myfile)

    def _print_only_columns(self, index, myfile):
        """Prints the columns and data types only in either the df1 or df2 datasets"""
        columns = getattr(self, 'columns_only_{index}'.format(index=index))
        df = getattr(self, index)

        if not columns:  # If there are no columns only in this df, don't display this section
            return

        max_length = max([len(col) for col in columns] + [11])
        format_pattern = '{{:{max}s}}'.format(max=max_length)

        print("\n****** Columns In {} Only ******".format(index.title()), file=myfile)
        ...
        for df1_row, df1_type in df1_columns_dict.items():
            if df1_row in df2_columns_dict:
                if df1_type not in df2_columns_dict.get(df1_row):
                    col_schema_diff[df1_row] = dict(df1_type=df1_type,
                                                     df2_type=df2_columns_dict.get(df1_row))
    ...
    def rows_only_df1(self):
        ...
        if not self._rows_only_df1:
            ...
            sql_query = "select A.* from df1Table as A, df1Rows as B where {}".format(join_condition)
    ...
    def rows_only_df2(self):
        ...
        if not self._rows_only_df2:
            df2_rows = self._get_unq_df2_rows()
            ...
            sql_query = "select A.* from df2Table as A, df2Rows as B where {}".format(where_condition)
    ...
    def _print_row_summary(self, myfile):
        ...
        print('Number of rows in df1 but not df2: {}'.format(df1_cnt - self.common_row_count), file=myfile)
        print('Number of rows in df2 but not df1: {}'.format(df2_cnt - self.common_row_count), file=myfile)
        print('Number of duplicate rows found in df1: {}'.format(df1_with_dup_cnt - df1_cnt), file=myfile)
        print('Number of duplicate rows found in df2: {}'.format(df2_with_dup_cnt - df2_cnt),
              file=myfile)
    ...
    def _print_schema_diff_details(self, myfile):
        ...
        for df1_column, types in schema_diff_dict.items():
            ...
            print((format_pattern + '  {:13s}  {:13s}').format(
                df1_column, df2_column, types['df1_type'], types['df2_type']), file=myfile)
    ...
    def _print_row_matches_by_column(self, myfile):
        ...
        df2_name_max = max([len(self._df1_to_df2_name(key)) for key in columns_with_any_diffs] + [19])
        ...
        for column_name, column_values in sorted(self.columns_match_dict.items(), key=lambda i: i[0]):
            ...
            if num_mismatches or num_known_diffs:
                output_row = [column_name, df2_column, df1_types.get(column_name),
                              df2_types.get(column_name), str(num_matches), str(num_mismatches)]
